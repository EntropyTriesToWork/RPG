...
public class PlayerController : BaseEntity
{
    ...
    [FoldoutGroup("ReadOnly")] [ReadOnly] public float lastTimeTookDamage;
    [FoldoutGroup("ReadOnly")] [ReadOnly] public int totalDashesMade;
    [FoldoutGroup("ReadOnly")] [ReadOnly] public int totalJumpsMade;
    [FoldoutGroup("ReadOnly")] [ReadOnly] public int totalFruitCollected;
    ...
    public override void Awake()
    {
        ...
        if (LevelManager.Instance != null) { LevelManager.Instance.pc = this; }
    }
    void FixedUpdate()
    {
        CheckForGround();
    ...
    public override void BodyTakeDamage(DamageReport report)
    {
        base.BodyTakeDamage(report);
    ...
    public void CheckShouldBounceOffEnemyHead()
    {
        if (!grounded && _rb.velocity.y < 0f)
        {
            ...
            if (hits.Length > 0)
            {
                ...
                foreach (var hit in hits)
                {
                    hit.GetComponent<BaseEnemy>().StompedOn();
                }
            }
        }
    }
